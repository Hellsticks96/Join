...
function checkForAllSubtasksDone(currentProgress, progressBar, arrayToSearch, popUpCheck){
    if (currentProgress > 99.9) {
        ...
        progressBar.classList.remove('all-subtasks-done-color');
        if (popUpCheck) {
            for (let i = 0; i < arrayToSearch.length; i++) {
                let checkbox = document.getElementById(`subtaskCheckbox${i}`);
                checkbox.classList.remove('all-subtasks-done-color');
            }  
        }
    }
}
...
async function openAddTask(status){
    ...
    if (window.innerWidth < 901) {
        ...
        addTaskOverlay.classList.remove('d-none');
        renderDate();
        setTimeout(slideAddTaskOverlay, 75);     
        if(status){
...
async function closeAddTask(){
    ...
    if (window.innerWidth < 901) {
        ...
        await slideAddTaskOverlay();
        setTimeout(hideOrShowAddTask , 125);
    }
